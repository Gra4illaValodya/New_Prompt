...
- Important: If the “deal_description” is repeated in “deal_type”: “SALES_PRICE” and “deal_type”: “REGULAR_PRICE”, replace it with Null in “SALES_PRICE”.
- IMPORTANT If duplicates appear in the "deal_type": "REGULAR_PRICE" and "deal_type": "SALES_PRICE" for the same group, save only in "deal_description" in "REGULAR_PRICE" and replace it with Null in "SALES_PRICE".
...
- if the description for each deal, then you need to write without duplicates in the "main_format" in "deal" in "deal_description"
- “deal_type": “SPECIAL_PRICE” must have a price
- Ensure base unit information (e.g., "1 kg = 8,58 €") is only in "deal_pricebybaseunit" and removed from "deal_description" in "main_format" in "deal".
- Ensure no duplicates exist in "main_format" in "deal" in "deal_description" or "deal_pricebybaseunit" across all deals in "main_format" in "deal".
- The deal_description field must be unique to the deal_type, and to duplicate the value in the “main_format” in the “deal” in the “deal_type”: “SALES_PRICE”, set it to Null.
- "product_name" must always have a value.
